<template>
  <div class="app-container">
	<div class="filter-container" style="margin-bottom: 15px;">
	  <el-select v-model="authStatus"  size="medium" placeholder="用户身份认证状态 " clearable class="filter-item" style="width: 180px">
	  ...
	  <el-button class="filter-item" type="primary"  size="medium" icon="el-icon-search" @click="searchData">
	    搜索
	  </el-button>
	</div>
    ...
    >
      ...
      <el-table-column label="用户身份认证状态" width="150" align="center">
        <template slot-scope="scope">
          <el-tag type="warning" v-if="scope.row.identityAuth=='EXAMINE'">审核中</el-tag>
          <el-tag type="success" v-if="scope.row.identityAuth=='PASS'">已通过</el-tag>
          <el-tag type="danger" v-if="scope.row.identityAuth=='REFUSE'">已拒绝</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="130" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <router-link :to="'/example/examineinfo/'+scope.row.id+'/3'">
...
import { getIdentityList,getuserIdentityInfo,putuserIdentityExamine } from '@/api/table'
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      getIdentityList(data).then(response => {
